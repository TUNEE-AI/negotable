...
// 확인 체크리스트 게이트의 "다시 묻기" 호출은 첫 호출이 이 시간 안에 끝났을 때만 한다.
// 첫 호출이 이미 오래 걸렸다면 한 번 더 부르면 사용자 대기가 1분을 넘기므로 건너뛴다.
const gateRetryBudgetMs = () => Number(process.env.GATE_RETRY_BUDGET_MS) || 20_000;
...
// 최근 대화 턴의 소요 시간(메모리에만, 대화 내용 없음). /api/health 에서 확인용.
const recent = [];
export const recentChatTimings = () => recent.slice();

// 숨은 "생각"을 끈 호출. 이 앱의 작업(대화 정리·ITEM 문장 작성)에는 긴 추론이 필요 없고,
// 켜두면 매 호출이 수 초~수십 초 늘어난다.
const fastLlm = (llm) => (req) => llm({ ...req, noThinking: true });
...
export async function chatTurn(llm, body) {
  ...
  let calls = 0;
  // 대화 턴은 짧은 답 + 바뀐 필드만 쓰는 가벼운 작업이라 effort를 낮춰 응답 시간을 줄인다(지원 모델만 적용).
  const counted = (req) => (calls++, llm({ ...req, effort: 'low', noThinking: true }));
  ...
  let state = mergeState(prev, out.state);
  ...
  if (phase === 'intake' && out.next_action === 'summarize' && userTurns < 7) {
    ...
    if (pending.length && Date.now() - started > gateRetryBudgetMs()) {
      console.log(`[chat] 첫 응답이 오래 걸려 확인 게이트 재요청을 건너뜀 (남은 항목: ${pending.join(', ')})`);
      ...
      state = mergeState(prev, out.state);
    }
  }
  ...
  const sec = Number(((Date.now() - started) / 1000).toFixed(1));
  console.log(`[chat] ${action} · AI 호출 ${calls}회 · 총 ${sec}s`);
  recent.push({ at: new Date().toISOString(), action, calls, sec });
...
export async function createItems(llm, body) {
  ...
  const out = await callGuarded(fastLlm(llm), {
