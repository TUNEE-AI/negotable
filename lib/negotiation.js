...
export async function chatTurn(llm, body) {
  ...
  // 1회 다시 판단하게 한다. 제안에 필요 없는 항목은 묻지 않고 not_applicable로 정리해 바로
  // 넘어갈 수 있고, 꼭 필요한 것이 남았을 때만 하나를 묻는다. (충분히 대화했으면 통과)
  if (phase === 'intake' && out.next_action === 'summarize' && userTurns < 7) {
    const pending = pendingCoverage(state);
    if (pending.length) {
      out = await call(
        system +
          `\n\n## 정정 지시\n아직 확인하지 않은 항목이 있습니다: ${pending.join(', ')}. 각 항목을 다시 판단하세요. 이미 사용자가 말했거나 대화에서 명백하면 known으로, 이 협상의 제안에 필요 없음이 분명하면 not_applicable로 표시하세요. 그렇게 모두 정리되면 그대로 "summarize"를 선택해도 됩니다. 제안을 쓰는 데 꼭 필요한데 아직 모르는 것이 남아 있다면 정리 카드로 넘어가지 말고(next_action은 "ask"), 그중 가장 중요한 하나만 자연스럽게 물어보세요.`,
      );
