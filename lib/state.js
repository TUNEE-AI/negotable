...
//
// 매 턴마다 LLM은 "바뀐 필드만" 돌려주고(응답 속도를 위해), 서버가 이전 상태에 합쳐 검증·정리한 뒤
...
export const STATE_SCHEMA = {
  type: 'object',
  description: '이번 턴에 바뀐 필드만(바뀐 필드는 그 필드의 값 전체). 생략한 필드는 이전 값이 유지된다. coverage는 항상 포함.',
  properties: {
    ...
    coverage: {
      type: 'object',
      description:
        '정리 전 확인 체크리스트. 항목별로 known=사용자가 말함, unknown=물었는데 모르거나 아직 안 정함, not_applicable=이 협상에는 해당 없음이 사용자 이야기에서 명백함, not_asked=아직 확인하지 않음.',
...
/**
 * LLM이 돌려준 "바뀐 필드"를 이전 상태에 합친다. 생략된 필드는 이전 값 유지.
 * 빈 값("" / [])도 "변경 없음"으로 본다 — 일부 모델은 생략 대신 모든 필드를 빈 값으로 채워 보내므로,
 * 이를 그대로 받으면 이미 모은 내용이 지워진다. (처음부터 다시 하려면 화면의 새로 시작 버튼을 쓴다.)
 */
export function mergeState(prev, delta) {
  const d = delta && typeof delta === 'object' ? delta : {};
  const merged = { ...prev };
  const isEmpty = (v) => v == null || (typeof v === 'string' && !v.trim()) || (Array.isArray(v) && v.length === 0);
  for (const [k, v] of Object.entries(d)) if (k !== 'items' && k !== 'coverage' && !isEmpty(v)) merged[k] = v;
  merged.coverage = { ...(prev?.coverage || {}), ...(d.coverage && typeof d.coverage === 'object' ? d.coverage : {}) };
  return normalizeState(merged, prev);
}
